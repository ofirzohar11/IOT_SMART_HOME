...
class TempSensorWindow(EmulatorWindow):
    ...
    def _build_ui(self):
        ...
        self.tempValue.setStyleSheet(
            'color: %s; font-family: %s; font-size: 34px; font-weight: bold; '
            'background: transparent; border: none;' % (ui.ACCENT, ui.FONT))

        self.humValue = QLabel('--')
        self.humValue.setAlignment(Qt.AlignCenter)
        self.humValue.setStyleSheet(
            'color: %s; font-family: %s; font-size: 34px; font-weight: bold; '
        ...
        self.modelLabel = ui.label('idle', size=11, color=ui.TEXT_DIM,
                                   align=Qt.AlignCenter)
        grid.addWidget(self.modelLabel, 2, 0, 1, 2)

        controls = ui.make_panel()
        ...
        self.ambientLabel = ui.label('Room temperature: 22 °C', size=11, color=ui.TEXT_DIM)
        ...
        self.ambientSlider.valueChanged.connect(
            lambda v: self.ambientLabel.setText('Room temperature: %d °C' % v))
        ambient_row.addWidget(self.ambientLabel)
        ambient_row.addWidget(self.ambientSlider, stretch=1)
        box.addLayout(ambient_row)

        self.faultCheck = QCheckBox('Inject cooling failure (compressor has no effect)')
    ...
    def _update_readouts(self, cooling_failed):
        ...
        self.tempValue.setStyleSheet(
            ...
            % (ui.OK if in_band else ui.ALARM, ui.FONT))
